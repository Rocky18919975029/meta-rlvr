...
export SMOKE_INNER_ITERATIONS="${SMOKE_INNER_ITERATIONS:-2}"
export SMOKE_OUTER_ITERATIONS="${SMOKE_OUTER_ITERATIONS:-2}"
...
echo "${submission}"
echo "configuration: problems=${SMOKE_PROBLEM_BATCH_SIZE} rollout_problem_batch=${SMOKE_ROLLOUT_PROBLEM_BATCH_SIZE} gradient_problem_microbatch=${SMOKE_PROBLEM_MICRO_BATCH_SIZE} vjp_forward_batch=${SMOKE_FIRST_ORDER_VJP_FORWARD_BATCH_SIZE:-1} token_jvp_batch=${SMOKE_TOKEN_JVP_RESPONSE_MICRO_BATCH_SIZE:-1} token_credit=${SMOKE_TOKEN_CREDIT_PARAMETERIZATION:-scaled_arctan}/${SMOKE_TOKEN_CREDIT_MAX:-1.0} token_meta_gradient=${SMOKE_TOKEN_META_GRADIENT_MODE:-gradient_alignment} max_new_tokens=3072 train_K=${SMOKE_SUPPORT_GROUP_SIZE}/${SMOKE_QUERY_GROUP_SIZE} validation_K=${SMOKE_VALIDATION_SUPPORT_GROUP_SIZE}/${SMOKE_VALIDATION_QUERY_GROUP_SIZE} train_sampling=${SMOKE_TEMPERATURE:-1.0}/${SMOKE_TOP_P:-1.0}/${SMOKE_TOP_K:-0} validation_sampling=${SMOKE_VALIDATION_TEMPERATURE:-1.0}/${SMOKE_VALIDATION_TOP_P:-0.7}/${SMOKE_VALIDATION_TOP_K:-0} losses=seq_meta:${SMOKE_META_COEFFICIENT:-1.0}/token_meta:${SMOKE_TOKEN_META_COEFFICIENT:-0.0}/bce:${SMOKE_BCE_COEFFICIENT:-1.0}/ranking:${SMOKE_RANKING_COEFFICIENT:-1.0} inner=${SMOKE_INNER_ITERATIONS:-2} outer=${SMOKE_OUTER_ITERATIONS:-2} rollout=${SMOKE_ROLLOUT_BACKEND} gpus=${SMOKE_GPUS} policy_batch=${SMOKE_POLICY_MICRO_BATCH_SIZE}/${SMOKE_POLICY_MAX_TOKENS_PER_MICRO_BATCH}tokens confidence_batch=${SMOKE_CONFIDENCE_MICRO_BATCH_SIZE}/${SMOKE_CONFIDENCE_MAX_TOKENS_PER_MICRO_BATCH}tokens deferred_sync=${SMOKE_DEFER_CONFIDENCE_GRADIENT_SYNC:-0} optimizer_offload=${SMOKE_OFFLOAD_CONFIDENCE_OPTIMIZER:-0} component_gradient_norms=${SMOKE_LOG_COMPONENT_GRADIENT_NORMS:-0} resume=${SMOKE_RESUME_FROM_CHECKPOINT:-none}"
